...
- **Post-restart Job failures now surfaced (no operator-side retry)**: a Failed post-restart Job (same config checksum) is left in place — the operator sets `PostRestartJobSucceeded=False` on the gateway and emits a one-time Warning event (`PostRestartJobFailed`), guarded so repeated reconciles of an unresolved failure don't spam events; success sets `PostRestartJobSucceeded=True`. Retrying transient failures is the job script's own responsibility (plus the Job's `backoffLimit`), not the operator's — a stuck Failed Job needs a manual `kubectl delete job` (or a config change that rolls a new checksum) to re-fire.
