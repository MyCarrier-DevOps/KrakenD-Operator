...
- **Post-restart Job retry with bounded attempts**: a Failed Job (same config checksum) is now deleted and recreated up to 3 attempts (tracked via a Job annotation and durably checkpointed in `status.lastPostRestartJobAttempt` so an operator crash mid-retry resumes rather than resetting the count), instead of being left stuck forever. At the retry cap, the Job is left in place and a terminal `PostRestartJobSucceeded=False` condition plus a Warning event are surfaced; success sets `PostRestartJobSucceeded=True`.
